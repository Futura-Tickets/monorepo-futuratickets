...
import { ClockCircleOutlined, LoadingOutlined, SendOutlined, SwapOutlined, TagOutlined } from "@ant-design/icons";

// INTERFACES
import { Sale, TicketStatus } from "@/shared/interfaces";

// STYLES
import "./OrderSaleItem.scss";

export default function OrderSaleItem({ sale }: { sale: Sale }) {
    ...
    return (
        <div className="order-sale-item-container">
            <div className="order-sale-item-content">
                <div className="details">
                    {sale.isResale && <SwapOutlined onClick={() => viewTicketDetails(`/events/${sale.event}/ticket/${sale._id}`)}/>}
                    {sale.isTransfer && <SendOutlined onClick={() => viewTicketDetails(`/events/${sale.event}/ticket/${sale._id}`)}/>}
                    {(!sale.isResale && !sale.isTransfer) && <TagOutlined onClick={() => viewTicketDetails(`/events/${sale.event}/ticket/${sale._id}`)}/>}
                </div>
