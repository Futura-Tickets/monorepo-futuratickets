        ...
        <div className="payments-header-actions">
            {actions.filter && (
                <Tooltip placement="bottom" title="Filter" color={color}>
                    <div className={"payments-header-action " + (filterState ? "active" : "")} onClick={() => setFilterState && setFilterState(!filterState)}>
                        <FilterOutlined />
                    </div>
                </Tooltip>
            )}
            {actions.access && (
                <Tooltip placement="bottom" title="Access" color={color}>
                    <div className={"payments-header-action " + setActiveAction('/access')} onClick={() => navigateTo(`/events/${pathname.split("/")[2]}/access`)}>
                        <LoginOutlined />
                    </div>
                </Tooltip>
            )}
            {actions.resale && (
                <Tooltip placement="bottom" title="Resales and Transfers" color={color}>
                    <div className={"payments-header-action " + setActiveAction('/resale')} onClick={() => navigateTo(`/events/${pathname.split("/")[2]}/resale`)}>
                        <SwapOutlined />
                    </div>
                </Tooltip>
            )}
            {actions.info && (
                <Tooltip placement="bottom" title="Event" color={color}>
                    <div className={"payments-header-action " + setActiveAction('/info')} onClick={() => navigateTo(`/events/${pathname.split("/")[2]}/info`)}>
                        <InfoCircleOutlined />
                    </div>
                </Tooltip>
            )}
            ...
            {actions.edit && (
                <Tooltip placement="bottom" title="Edit" color={color}>
                    <div className="payments-header-action" onClick={() => setEventEditModal && setEventEditModal(true)}>
                        <EditOutlined/>
                    </div>
                </Tooltip>
            )}
            {actions.delete && (
                <Tooltip placement="bottom" title="Delete" color={color}>
                    <div className="payments-header-action" onClick={() => setEventRemoveModal && setEventRemoveModal(true)}>
                        <DeleteOutlined />
                    </div>
                </Tooltip>
            )}
