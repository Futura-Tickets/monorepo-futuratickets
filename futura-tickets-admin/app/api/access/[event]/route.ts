...
export async function GET(request: NextRequest, { params }: { params: Promise<{ event: string }>}) {
    try {
        ...
        // Make the API call
        const res = await fetch(`${process.env.NEXT_PUBLIC_FUTURA}/admin/events/access/${urlParams.event}`, {
            method: "GET",
            headers: {
...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ eventAddress: string }>}) {
    try {
        ...
        // Make the API call
        const res = await fetch(`${process.env.NEXT_PUBLIC_FUTURA}/admin/events/access/${urlParams.eventAddress}`, {
            method: "PATCH",
            headers: {
                "Content-Type": "application/json",
                "Authorization": request.headers.get('authorization')!
