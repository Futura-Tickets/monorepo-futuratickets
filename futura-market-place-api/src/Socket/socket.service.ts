...
// INTERFACES
import { EmitAccess, EmitOrder, EmitTransfer, EmitResale } from 'src/Sales/sales.interface';

@Injectable()
export class SocketService {

    public socket = new Server({ // Will attach to the main HTTP server
        pingTimeout: 10000,
        cors: {
            origin: '*',
            methods: ['GET', 'POST']
    ...
    private socketMarketPlace: string;
    ...
    constructor(
        ...
        this.socketMarketPlace = configService.get('SOCKET_MARKET_PLACE');

        // Only initialize Azure Socket.IO if connection string is provided
        if (this.socketMarketPlace) {
            useAzureSocketIO(this.socket, {
                hub: 'Centro',
                connectionString: this.socketMarketPlace
            });
        } else {
            console.warn('⚠️  SOCKET_MARKET_PLACE not configured. WebSocket functionality will be limited.');
        }
    ...
    public handleDisconnection(promoter: string, client: Socket) {
        this.connectedPromoter.set(promoter, this.connectedPromoter.get(promoter)?.filter((promoterClientId: string) => promoterClientId != client.id)!);
        client.disconnect();
    };
    ...
    public emitUserCreated(promoter: string, notificationId: string): void {
        this.socket.to(this.connectedPromoter.get(promoter.toString())).emit('user-created', notificationId);
    };
}