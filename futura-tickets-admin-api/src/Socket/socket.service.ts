...
export class SocketService {
  ...
  public handleDisconnection(promoter: string, client: Socket) {
    ...
    if (clients) {
      this.connectedPromoter.set(
        promoter,
        clients.filter(
          (promoterClientId: string) => promoterClientId != client.id,
        ),
      );
    }
    client.disconnect();
  ...
  public emitTicketResale(promoter: string, order: string): void {
    this.socket
      .to(this.connectedPromoter.get(promoter.toString()))
      .emit('ticket-resale', order);
  }

  public emitCancelResale(promoter: string, order: string): void {
    this.socket
      .to(this.connectedPromoter.get(promoter.toString()))
      .emit('cancel-resale', order);
  }

  public emitOrderCreatedNotification(
    promoter: string,
    orderIdNotification: string,
  ): void {
    this.socket
      .to(this.connectedPromoter.get(promoter.toString()))
